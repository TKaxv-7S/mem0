...
The [TypeScript OSS SDK](/open-source/features/reranker-search#typescript-sdk) (`mem0ai/oss`) ships the Zero Entropy reranker under the provider name `zeroentropy` (one word, unlike the Python `zero_entropy`). It reads the key from config or `ZEROENTROPY_API_KEY` and defaults to the `zerank-1` model.
