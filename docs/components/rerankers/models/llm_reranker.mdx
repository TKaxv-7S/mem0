...
The [TypeScript OSS SDK](/open-source/features/reranker-search#typescript-sdk) (`mem0ai/oss`) ships the LLM reranker under the provider name `llm`. It reuses the Memory's main `llm` unless you pass a dedicated one under `config.llm` — no extra install needed.
...
const memory = new Memory({
  ...
  reranker: {
    provider: "llm",
    // Reuses the `llm` above. To override with a dedicated model:
