...
pnpm add zeroentropy        # zeroentropy
pnpm add @huggingface/transformers   # sentence_transformer, huggingface
# llm reuses your configured LLM provider — no extra install
```

### Hosted rerankers (Cohere, Zero Entropy)

Both call a hosted API and read their key from config or the provider's environment variable (`COHERE_API_KEY`, `ZEROENTROPY_API_KEY`).
...
const memory = new Memory({
  reranker: {
    ...
    config: { apiKey: process.env.ZEROENTROPY_API_KEY },
  },
});
```
...
To score with an LLM instead of a dedicated reranker, use the `llm` provider. It reuses the Memory's main `llm` unless you name a dedicated one:
...
const memory = new Memory({
  ...
  reranker: {
    provider: "llm",
    // Reuses the `llm` above. To override, pass a dedicated one:
    // config: { llm: { provider: "openai", config: { model: "gpt-5-mini", apiKey: "..." } } }
    config: {},
  },
});
...
| --- | --- | --- |
| `cohere` | `rerank-v3.5` | `apiKey`, `model`, `topN` |
| `zeroentropy` | `zerank-1` | `apiKey`, `model`, `topN` |
| `sentence_transformer` | `Xenova/ms-marco-MiniLM-L-6-v2` | `model`, `device`, `maxLength`, `normalize`, `topN` |
| `huggingface` | `Xenova/bge-reranker-base` | `model`, `device`, `maxLength`, `normalize`, `topN` |
| `llm` | reuses the Memory's `llm` | `llm` (optional dedicated provider), `topN` |
