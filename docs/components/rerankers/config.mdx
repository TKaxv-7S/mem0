...
The self-hosted [TypeScript SDK](/open-source/features/reranker-search#typescript-sdk) (`mem0ai/oss`) supports the same five providers. Config keys are camelCase (`apiKey`, `topN`, `maxLength`) and each provider's SDK is a peer dependency you install per reranker.

| Provider | Install | Default model | Key config fields |
| --- | --- | --- | --- |
| `cohere` | `pnpm add cohere-ai` | `rerank-v3.5` | `apiKey`, `model`, `topN` |
| `zeroentropy` | `pnpm add zeroentropy` | `zerank-1` | `apiKey`, `model`, `topN` |
| `sentence_transformer` | `pnpm add @huggingface/transformers` | `Xenova/ms-marco-MiniLM-L-6-v2` | `model`, `device`, `maxLength`, `normalize`, `topN` |
| `huggingface` | `pnpm add @huggingface/transformers` | `Xenova/bge-reranker-base` | `model`, `device`, `maxLength`, `normalize`, `topN` |
| `llm` | — (reuses your `llm`) | reuses the Memory's `llm` | `llm` (optional dedicated provider), `topN` |
...
const memory = new Memory({
  reranker: {
    ...
    config: { apiKey: process.env.ZEROENTROPY_API_KEY, topN: 5 },
  },
});
```
